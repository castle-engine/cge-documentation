...
for adding shader extensions \cite{ogre:shader}. Its idea is similar
...
While not necessary, it's often comfortable to wrap a texture generation
function inside the \texttt{ShaderTexture} node. Since \texttt{ShaderTexture}
is treated much like other X3D texture nodes by the browser,
you can provide texture coordinates (explicit or generated) for this texture.
...
and \texttt{"BOUNDS2D"} and \texttt{"BOUNDS3D"} are also valid names
...
This was possible thanks to the fact that an effect can define
new plug names, that can be used by the following effects.
