...
\texttt{Group} and \texttt{Transform}. This effect will apply
to all the shapes within given grouping node. The way it works
follows X3D conventions for pointing device sensor nodes and \texttt{LocalFog}.

The \texttt{LocalFog} is especially worth mentioning. Browser can implement
the \texttt{LocalFog} node as a prototype that expands to our
\texttt{Effect} node. This gives you a 100\% correct implementation
of the standard \texttt{LocalFog} node, as is trivially easy.
...
TODO: screenshot of volumetic fog with / without headlight,
with / without scene lights used.
