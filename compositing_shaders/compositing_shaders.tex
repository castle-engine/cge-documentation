...
1. I will probably remove name field, and replace it with SFString type =
FRAGMENT | VERTEX. Plug names will be automatically found by magic
function names PLUG\_xxx (followed by whitespace or "(") within source code,
not by given plug names.
This allows cleaner code, as user can write himself function declaration.

2. Also, this cooperates nicely with idea to place each effect is a separate
compilation unit for GLSL.
...
each effect in such separate unit. This allows for cleaner error detection
...
the beginning of given part.

TODO: for now we have different implementation, with
