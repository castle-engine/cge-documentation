...
\author{paper1013} % TODO: submission id
...
An oldest solution to combine effects, used even before the shading languages,
...
AnySL \cite{anysl} allows to integrate renderer
shaders with user shaders. We strive to do something more and allow effortless
integration between many user shaders. We also try to be simpler as we do not
...
then the internal renderer effects will be even added to your own
...
We have also considered and rejected a different approach to define
\texttt{PLUG\_} functions. In that rejected approach,
we did not look at any special function names
in a shader code, instead the plug name was defined in the separate field of
the \texttt{EffectPart}
...
in another field. Another idea was to split shader code into much more
