...
changes and improvements. See the webpage of own engine, and in
...
units'' idea is \textit{not} necessary for a successfull
...
(although a preprocessor directive like \#line could also be used for
...
\begin{itemize}
  ...
  \item You can also change the "inout" parameter to just "out", if
    ...
    this is usually not adviced, as it means that you disable previous
...
\begin{Verbatim}[commandchars=\\\{\},frame=single]
void PLUG_fragment_normal_eye(
  inout vec3 normal)
\end{Verbatim}

Modify the normal in eye space, at fragment shader.
This is where the traditional bump mapping work is done.
