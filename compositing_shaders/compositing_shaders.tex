...
% accepted for TPCG 2011.
% Copyright 2011 by The Eurographics Association.
...
%\author{paper1013} % submission id for anon sending
...
%% as we do not want to throw all the effects in one bag).
...
is below. This is a complete X3D file. You can save
...
The bold part of the above example indicates our extensions to X3D.
The italic part indicates GLSL code inside our extensions.
...
The GLSL function name \texttt{PLUG\_texture\_apply} is special,
it indicates that we use the \texttt{texture\_apply} plug.
This particular plug is called right after applying the textures,
and is the simplest way to ``just modify the pixel color''.
%% The \texttt{texture\_apply} plug point in
...
We have a reference of all the plugs available in our implementation
on \URL{http://vrmlengine.sourceforge.net/compositing_shaders.php}.
For each plug, like this \texttt{PLUG\_texture\_apply}, we define a list of parameters
...
The node specification below is presented in the same style
as in the X3D specification \cite{x3d:spec}.
...
current world time or a texture to the shader.
...
%% Detecting special \texttt{PLUG\_} function names inside the shader code
...
%% the \textit{separate compilation units} in case of GLSL implementation.
...
like the \texttt{ImageTexture}. This allows to to modify any X3D texture
by shader effects.
...
many times. For example, this is useful when desired calculation is naturally
...
New plug points can be defined inside the \texttt{Effect} nodes,
...
that don't support our extensions.
...
\URL{http://vrmlengine.sourceforge.net/compositing_shaders.php}.
